...
public class LoginValidator {
    /*
     * the constants have been created as static to ensure their values do not
     * change during runtime, this can lead to
     * disconnecting from the database if they do change.
     */
    private Connection connection;
    private static final String DB_URL = "jdbc:mysql://cs2410-web01pvm.aston.ac.uk:3306/u_210131701_adminlogin"; // //
                                                                                                                 // info
                                                                                                                 // to
                                                                                                                 // the
                                                                                                                 // URL
                                                                                                                 ...
                                                                                                                 // the
                                                                                                                 // database
    // to provide an exact location
    // of where the database is
    private static final String DB_USERNAME = "u-210131701"; // info to the database username, to allow access
                                                             // to the
    // database
    private static final String DB_PASSWORD = "6c5FtT45Wq3Vofg"; // info to the database password, to allow access to
                                                                 // the
    // database
    ...
    public boolean login(String email, String password) throws ClassNotFoundException { /*
                                                                                         ...
                                                                                         * which is static,
                                                                                         * it also takes the email and
                                                                                         * password as parameters, this
                                                                                         * is
                                                                                         * to ensure they do not change
                                                                                         * during runtime.
                                                                                         */
        ...
        boolean loginSuccessful = false; // a new variable created and set to false, this is to check if the login
                                         // details match the database

        try (DBLink link = new DBLink(DB_URL, DB_USERNAME, DB_PASSWORD); // create a new instance of auto closeable
                PreparedStatement preparedStatement = connection
                        .prepareStatement("SELECT email, password FROM Admins WHERE email = ? AND password = ?")) {
 ...
 * 
 * resultSet.next will set loginsucessful to true if there is at least one row
 * in
 ...
 * the close() method implemented by the Autocloseable interface is
 * automatically
